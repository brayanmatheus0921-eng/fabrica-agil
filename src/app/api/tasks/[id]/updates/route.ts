...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const task = await prisma.task.findFirst({ where: { id, companyId: DEV_COMPANY_ID }, include: { actionPlan: true } });
  ...
  const note = await prisma.evidenceOutput.upsert({ where: { id: key }, update: {}, create: { id: key, companyId: DEV_COMPANY_ID, taskId: id, type: "NOTE", label: metadata.category, textValue: metadata.text, metadata } });
