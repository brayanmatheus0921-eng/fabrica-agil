...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 try{
  const records=await prisma.$transaction(async tx=>{
   await tx.$queryRaw`SELECT id FROM "ActionPlan" WHERE id = (SELECT "actionPlanId" FROM "Task" WHERE id = ${id} AND "companyId" = ${(await requireAuth()).companyId}) FOR UPDATE`;
   await tx.$queryRaw`SELECT id FROM "Task" WHERE id = ${id} AND "companyId" = ${(await requireAuth()).companyId} FOR UPDATE`;
   const task=await tx.task.findFirst({where:{id,companyId:(await requireAuth()).companyId},include:{actionPlan:true}});
   ...
   const rows=await tx.evidenceOutput.findMany({where:{taskId:id,companyId:(await requireAuth()).companyId},orderBy:{createdAt:"asc"}}),existing=readRecords(rows);
   ...
   if(input.intent==="UNDO"){
    ...
    await tx.evidenceOutput.create({data:{id:recordId,companyId:(await requireAuth()).companyId,taskId:id,type:"NOTE",label:"Correção de registro",metadata:{kind:"UNDO",targetId:last.id}}});
   }else{
    ...
    await tx.evidenceOutput.create({data:{id:recordId,companyId:(await requireAuth()).companyId,taskId:id,type:"NOTE",label:guide.recording.title,textValue:JSON.stringify(metadata),metadata:metadata as never}});
   }
