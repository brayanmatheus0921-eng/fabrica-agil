...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const existing = await prisma.workspaceArtifact.findFirst({ where: { id, companyId: DEV_COMPANY_ID }, select: artifactSelect });
    ...
    const updated = await prisma.workspaceArtifact.updateMany({ where: { id, companyId: DEV_COMPANY_ID, revision: body.revision }, data });
