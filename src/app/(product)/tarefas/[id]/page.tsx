...
export default async function TaskPage({params}:{params:Promise<{id:string}>}){
 ...
 const notes: TaskNoteView[] = evidence.flatMap(row => { const p = taskNoteSchema.safeParse(row.metadata); return p.success && !Object(row.metadata).voided ? [{ id: row.id, category: p.data.category, text: p.data.text, at: row.createdAt.toISOString() }] : []; }).reverse();
