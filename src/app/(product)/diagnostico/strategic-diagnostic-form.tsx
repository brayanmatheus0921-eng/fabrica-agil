...
const answerOptions = [
  { value: "YES", label: "Sim", detail: "Funciona na rotina" },
  { value: "PARTIAL", label: "Em parte", detail: "Funciona, mas falha às vezes" },
  { value: "NO", label: "Não", detail: "Ainda não funciona" },
];
const impactOptions = [
...
export function StrategicDiagnosticForm({ sessionId, questions, currentIndex, initialValue, initialNotes, error }: { sessionId: string; questions: Question[]; currentIndex: number; initialValue?: unknown; initialNotes: string; error?: string }) {
  ...
  return <form action={saveStrategicAnswer} className="flex min-h-0 flex-1 flex-col gap-[clamp(.3rem,1.2dvh,.65rem)]">
    ...
    <fieldset className="min-h-0 flex-1 overflow-y-auto rounded-2xl border bg-white px-[clamp(.8rem,3vw,1.7rem)] py-[clamp(.6rem,2dvh,1.2rem)]"><legend className="sr-only">{question.prompt}</legend>
      ...
      <div className={`mt-4 grid gap-2 ${impactQuestion ? "grid-cols-1" : "sm:grid-cols-3"}`}>{options.map((option, optionIndex) => <label key={option.value} className="flex cursor-pointer items-center gap-3 rounded-xl border p-3 font-normal has-[:checked]:border-primary has-[:checked]:bg-accent has-[:checked]:text-primary"><input required type="radio" name="value" value={option.value} defaultChecked={initialValue === option.value}/><span className="grid size-6 shrink-0 place-items-center rounded-full bg-[#edf1eb] text-[10px] font-normal">{optionIndex + 1}</span><span><span className="text-sm font-normal">{option.label}</span>{option.detail ? <span className="mt-0.5 block text-[11px] font-normal text-muted">{option.detail}</span> : null}</span></label>)}</div>
