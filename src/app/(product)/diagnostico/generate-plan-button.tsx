...
export function GeneratePlanButton({ sessionId, label = "Montar plano com o COO", className = "rounded-xl bg-primary px-5 py-3 text-sm font-bold text-white disabled:opacity-60" }: { sessionId: string; label?: string; className?: string }) {
  const [state, action, pending] = useActionState(generateDiagnosticPlan, { error: null });
  return <form action={action} className="space-y-3">
    <input type="hidden" name="sessionId" value={sessionId} />
    <button disabled={pending} className={className}>
      {pending ? "Abrindo conversa…" : label}
    </button>
