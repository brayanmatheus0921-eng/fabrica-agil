...
export default async function NewDiagnosticPage({ searchParams }: { searchParams: Promise<{ q?: string; error?: string; session?: string; review?: string }> }) {
  ...
  return <main className="h-dvh min-h-[520px] overflow-y-auto bg-[#f5f3ef] px-3 py-3 sm:px-8"><div className="mx-auto grid h-full max-w-5xl grid-rows-[3rem_minmax(0,1fr)] gap-2"><header className="flex items-center justify-between"><Link href="/dashboard" className="text-lg font-black text-[#0b1320]">FÁBRICA <span className="text-primary">ÁGIL</span></Link><Link href="/diagnostico" className="rounded-xl border bg-white px-3.5 py-2 text-sm font-bold text-[#59616d]">Sair por agora</Link></header><div className="flex min-h-0 items-center justify-center">
    ...
    : session ? <section className="flex h-full max-h-[760px] w-full min-h-0 flex-col overflow-hidden rounded-3xl border bg-white p-[clamp(.75rem,2.2dvh,1.25rem)] shadow-xl"><div className="mb-2 flex shrink-0 items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[#f5e8d8] text-primary"><Target className="size-5"/></span><div><p className="text-xs font-bold uppercase tracking-[.14em] text-primary">Diagnóstico operacional</p><p className="text-sm text-muted">{company.name}</p></div></div><StrategicDiagnosticForm sessionId={session.id} questions={template.questions} currentIndex={index} initialValue={current?.value} initialNotes={current?.notes ?? ""} error={params.error}/></section>
