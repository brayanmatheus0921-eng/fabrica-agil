...
function preview(a: CooAction, snapshot: unknown): { summary: string; details: string[] } {
  ...
  switch (a.type) {
    ...
    case "workshop.patch": return { summary: `Posso salvar esta revisão do plano · ${STAGE_LABELS[a.patch.stage]}?`, details: [a.patch.summary, ...a.patch.confirmedFacts.map(f => `Fato informado: ${f.statement}`), ...a.patch.hypotheses.map(h => `Hipótese: ${h}`), ...(a.patch.decision ? [`Prioridade: ${a.patch.decision.primaryTitle} — ${a.patch.decision.reason}`] : []), ...(a.patch.plan ? [`Objetivo: ${a.patch.plan.objective}`, ...a.patch.plan.initiatives.flatMap(i => [i.title, ...i.actions.map(x => `${x.what} · ${x.who} · ${x.whenDays} dias\nPor quê: ${x.why}\nOnde: ${x.where}\nComo: ${x.how}\nCusto: ${x.howMuch}\nIndicador: ${x.indicator}\nBase: ${x.baseline}\nMeta: ${x.target}\nEntrega: ${x.proof}\nRevisão: ${x.reviewCadence}\nGuia: ${guidePreview(x.execution)}`)])] : []), "Salvar a revisão não inicia a execução das tarefas."] };
...
async function apply(db: DB, actor: ActionActor, proposal: CooActionProposal, a: CooAction): Promise<CooActionResult> {
  ...
  switch (a.type) {
    ...
    case "workshop.patch": {
      ...
      return { message: "Revisão salva com sua aprovação.", href: state.planId ? `/plano-de-acao?id=${state.planId}` : `/assistente?chat=${threadId}` };
    }
    case "artifact.save": {
