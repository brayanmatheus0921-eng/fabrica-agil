...
export function AssistantComposer({ value, onChange, onSend, onStop, busy, stopping, disabled, planning = false }: {
  value: string; onChange: (value: string) => void; onSend: () => void; onStop: () => void; busy: boolean; stopping: boolean; disabled?: boolean; planning?: boolean;
}) {
  const input = useRef<HTMLTextAreaElement>(null);
  return <form onSubmit={event => { event.preventDefault(); if (!busy && !disabled) onSend(); }} className="mx-auto max-w-[700px]">
    <div className="flex items-end gap-2 rounded-2xl border bg-white p-1.5 shadow-sm focus-within:ring-2 focus-within:ring-primary/20">
      <textarea ref={input} aria-label="Mensagem para o consultor" value={value} disabled={disabled} maxLength={6000} rows={1}
        placeholder={busy ? "Aguarde a resposta ou clique em Parar" : planning ? "Vamos construir seu plano. Responda ou peça uma sugestão…" : "Peça ao COO para organizar, registrar ou resolver…"}
        ...
        className="max-h-40 min-h-11 min-w-0 flex-1 resize-none bg-transparent px-3 py-2.5 text-base leading-6 outline-none sm:text-[13px] sm:leading-5" />
    ...
    <p className="mt-2 text-center text-[10px] text-muted">Enter envia · Shift + Enter quebra linha · Toda alteração depende da sua aprovação</p>
  </form>;
}
