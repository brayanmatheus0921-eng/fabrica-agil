...
type OverviewTask = { id: string; title: string; ownerName: string | null; status: string; dueAt: Date | null };
const taskStatusLabels: Record<string, string> = { BACKLOG: "Planejada", TODO: "A fazer", IN_PROGRESS: "Em andamento", BLOCKED: "Bloqueada", IN_REVIEW: "Em revisão", DONE: "Concluída", CANCELLED: "Cancelada" };
const taskDueLabel = (date: Date | null) => date ? new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "short", year: "numeric", timeZone: "America/Sao_Paulo" }).format(date) : "Sem prazo definido";
export function CooPlanOverview({plan,sourceDiagnosis,threadId,demo,approval,error}:{
  ...
  return <div className="mx-auto max-w-6xl space-y-7 pb-5">
    ...
    <section aria-label="Seu próximo passo" className="rounded-xl border border-[#d9e3ee] border-l-[3px] border-l-[#a17c55] bg-[#f1f5fa] p-5 sm:p-6">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center"><div className="max-w-2xl"><p className="text-xs font-semibold text-[#57708a]">{pending?"Antes de começar":active&&nextTask?"Seu próximo passo":allDone?"Hora de conferir o resultado":"Organize a próxima ação"}</p><h2 className="mt-2 text-lg font-semibold leading-7">{pending?"Confira o combinado. Você decide quando começar.":active&&nextTask?nextTask.title:allDone?"Concluir tarefas não é o mesmo que resolver o problema.":"Revise com o COO o que precisa acontecer agora."}</h2><p className="mt-2 text-sm leading-6 text-[#485b70]">{pending?"Veja uma iniciativa por vez no quadro abaixo. Depois de aprovar, os prazos e as tarefas são liberados.":active&&nextTask?"Abra a tarefa para conferir as instruções e registrar o andamento.":allDone?"Leve os registros ao COO para comparar o resultado com o ponto de partida.":"O plano permanece salvo. Nenhuma ação é iniciada por esta tela."}</p></div><Link href={nextHref} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 self-start rounded-lg bg-primary px-5 py-3 text-sm font-semibold text-white lg:self-center">{nextLabel}<ArrowRight aria-hidden="true" className="size-4"/></Link></div>
      {active&&nextTask?<dl className="mt-4 grid gap-3 border-t border-[#d9e3ee] pt-4 text-sm sm:grid-cols-3">{[["Responsável",nextTask.ownerName??"Não informado"],["Prazo atual",taskDueLabel(nextTask.dueAt)],["Situação",taskStatusLabels[nextTask.status]??nextTask.status]].map(([label,value])=><div key={label}><dt className="text-xs font-semibold text-[#57708a]">{label}</dt><dd className="mt-1 break-words text-[#485b70]">{value}</dd></div>)}</dl>:null}
    </section>
    <div className="border-b border-[#e2e6eb] pb-6"><p className="text-sm font-semibold text-primary">Objetivo deste ciclo</p><p className="mt-2 max-w-4xl text-sm leading-6 text-[#39485b]">{plan.objective}</p><div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted"><span className="inline-flex items-center gap-1.5"><CheckCircle2 aria-hidden="true" className="size-3.5"/>{parsed.data.initiatives.length} iniciativas {pending?"previstas":"no plano original"} · uma de foco principal</span><span className="inline-flex items-center gap-1.5"><CalendarDays aria-hidden="true" className="size-3.5"/>{pending?"Prazos contam após a aprovação":"Prazos atuais na lista de tarefas"}</span></div></div>
    ...
    {!pending?<section aria-label="Tarefas atuais deste projeto" className="rounded-xl border border-[#e2e6eb] bg-white p-5 sm:p-6">
      <h2 className="text-lg font-semibold">Tarefas atuais deste projeto</h2><p className="mt-1 text-sm text-muted">Dados atuais, incluindo alterações feitas depois da aprovação do plano.</p>
      {plan.tasks.length?<ol className="mt-4 divide-y divide-[#e2e6eb]">{plan.tasks.map(task=><li key={task.id} className="py-4"><Link href={`/tarefas/${task.id}`} className="inline-flex min-h-10 items-center gap-2 break-words text-sm font-semibold text-primary">{task.title}<ArrowRight aria-hidden="true" className="size-4 shrink-0"/></Link><dl className="mt-2 grid gap-3 text-sm sm:grid-cols-3">{[["Responsável",task.ownerName??"Não informado"],["Prazo",taskDueLabel(task.dueAt)],["Situação",taskStatusLabels[task.status]??task.status]].map(([label,value])=><div key={label}><dt className="text-xs text-muted">{label}</dt><dd className="mt-1 break-words">{value}</dd></div>)}</dl></li>)}</ol>:<p className="mt-4 text-sm text-muted">Este projeto está sem tarefas vinculadas.</p>}
    </section>:null}
