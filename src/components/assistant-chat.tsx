...
function DeleteConversationControl({ thread, action, disabled }: { thread: { id: string; title: string }; action: (form: FormData) => void | Promise<void>; disabled: boolean }) {
  const [confirming, setConfirming] = useState(false);
  if (!confirming) return <button type="button" disabled={disabled} onClick={() => setConfirming(true)} className="flex min-h-9 w-full items-center gap-2 rounded-lg px-2.5 text-left text-xs font-semibold text-red-700 hover:bg-red-50 disabled:opacity-30"><Trash2 className="size-3.5" />Excluir conversa</button>;
  return <form action={action} className="mt-1 space-y-2 border-t p-2 text-xs">
    <p>Excluir permanentemente “{thread.title}” e suas mensagens?</p>
    <input type="hidden" name="threadId" value={thread.id} />
    <input type="hidden" name="confirmation" value={thread.title} />
    <div className="flex gap-2"><button type="button" onClick={() => setConfirming(false)} className="min-h-9 flex-1 rounded-lg border px-2">Cancelar</button><button disabled={disabled} className="min-h-9 flex-1 rounded-lg bg-red-700 px-2 font-semibold text-white disabled:opacity-40">Confirmar exclusão</button></div>
  </form>;
}
...
export function AssistantChat({ messages: initialMessages, threads, currentThreadId, initialWorkshop, initialGenerationId, deleteAction, renameAction, newChatAction, disabled, defaultValue, suggestions, error, planningWorkspace = false, initialMemory }: {
  ...
  useEffect(() => {
    if (refreshedOnOpen.current || currentThreadId === "new") return;
    refreshedOnOpen.current = true;
    router.refresh();
  }, [currentThreadId, router]);
  useEffect(() => {
    if (busyRef.current || activeThreadIdRef.current !== currentThreadId || initialMessages.length < messages.length) return;
    setMessages(initialMessages);
    setMemory(initialMemory ?? emptyConversationMemory(planningWorkspace ? "PLAN" : "COO"));
    setState(initialWorkshop);
  }, [initialMessages, initialMemory, initialWorkshop, currentThreadId, planningWorkspace, busy, messages.length]);
  ...
  useEffect(()=>{if(activeThreadId==="new"||planningWorkspace)return;const controller=new AbortController();fetch(`/api/artifacts?threadId=${encodeURIComponent(activeThreadId)}`,{signal:controller.signal}).then(r=>r.ok?r.json():Promise.reject()).then(data=>setArtifacts(Object.fromEntries((data.artifacts as ArtifactView[]).map(a=>[a.id,a])))).catch(()=>{});return()=>controller.abort();},[activeThreadId,proposalRefresh,canvasRefresh,planningWorkspace]);
  useEffect(() => {
    ...
    if (decision && activeThreadIdRef.current !== "new") {
      try {
        ...
        if (pending.length === 1) {
          ...
          setMessages(old=>[...old,{id:approvalMessageId,role:"USER",content:text},{id:crypto.randomUUID(),role:"ASSISTANT",content:answer,proposalId:p.id}]);
  ...
  const onProposalApplied=(proposal:CooProposalView)=>{notifyCompanyDataChanged();setNotice("Ação aplicada com sua aprovação.");setCanvasRefresh(v=>v+1);const approvalId=crypto.randomUUID();readingAnchor.current=approvalId;follow.current=false;setMessages(old=>[...old,{id:approvalId,role:"USER",content:`Aprovei: ${proposal.summary}`},{id:crypto.randomUUID(),role:"ASSISTANT",content:proposal.result?.message??"Ação aplicada.",proposalId:proposal.id}]);router.refresh();fetch(`/api/assistant/proposals?threadId=${encodeURIComponent(activeThreadId)}`).then(r=>r.json()).then(data=>{if(data.workshop!==undefined)setState(data.workshop);if(data.memory)setMemory(data.memory);}).catch(()=>{});void resumeInterview(proposal);};
  ...
  const onOpenArtifact=(id:string)=>{if(canvasDirty.current&&!window.confirm("Descartar alterações não salvas?"))return;canvasDirty.current=false;setOpenArtifactId(id);setCanvasPanel(true);setRecordPanel(false);setPanel(false);};
  const approvalMessages = new Set(messages.map(m=>m.proposalId).filter((id):id is string=>Boolean(id)));
  const approvalArtifact = (message:Message) => {
    const id = proposals.find(p=>p.id===message.proposalId)?.result?.artifactId;
    const artifact = id ? artifacts[id] : null;
    return artifact ? <div className="space-y-3"><ArtifactPreview artifact={artifact}/><button type="button" onClick={()=>onOpenArtifact(id!)} className="inline-flex min-h-10 items-center rounded-lg bg-primary px-4 text-xs font-semibold text-white">Abrir para preencher</button></div> : null;
  };
  const dedicatedPlanning=planningWorkspace;

  return <div data-workspace={planningWorkspace?"plan":"coo"} className="coo-chat relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-white">
    <section className={`assistant-chat-pane relative flex min-w-0 flex-1 flex-col transition-[opacity,transform] duration-200 ease-out ${switchingChat ? "translate-y-1 opacity-0" : "translate-y-0 opacity-100"}`}>
      ...
      <div ref={scroll} data-empty={!messages.length} onScroll={() => { const node = scroll.current; if (node) { follow.current = node.scrollHeight - node.scrollTop - node.clientHeight < 90; setAway(!follow.current); } }} className="coo-conversation min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5 sm:px-8">
        <div className="mx-auto flex max-w-3xl flex-col gap-6">
          ...
          {proposals.some(p=>p.status==="PENDING"&&!attachedSources.has(p.sourceMessageId))?<AssistantProposals rows={proposals.filter(p=>p.status==="PENDING"&&!attachedSources.has(p.sourceMessageId))} busy={busy} onAdjust={onProposalAdjust} onApplied={onProposalApplied} onChanged={onProposalChanged} artifacts={artifacts} onOpenArtifact={onOpenArtifact} hideArtifactPreviewIds={approvalMessages}/>:null}
    ...
    {canvasPanel && !dedicatedPlanning ? <aside id="assistant-canvas" aria-label="Canvas do COO" className="absolute inset-y-0 right-0 z-20 w-full overflow-y-auto border-l bg-white p-4 shadow-xl sm:w-[min(90%,640px)] xl:static xl:w-[48%] xl:shrink-0 xl:shadow-none"><button aria-label="Fechar Canvas" className="mb-4 ml-auto flex min-h-10 items-center gap-2 rounded-lg border px-3 text-xs" onClick={() => { if (canvasDirty.current && !window.confirm("Descartar alterações não salvas?")) return; canvasDirty.current = false; setCanvasPanel(false); }}>Voltar à conversa<X className="size-4"/></button><ArtifactWorkspace key={`${activeThreadId}:${openArtifactId??""}`} threadId={activeThreadId} refreshKey={canvasRefresh} openArtifactId={openArtifactId} onDirtyChange={value => { canvasDirty.current = value; }} onArtifactChange={artifact=>setArtifacts(old=>({...old,[artifact.id]:artifact}))}/></aside> : null}
    <aside id="assistant-details" aria-label="Histórico de conversas" aria-hidden={!panel} className={`absolute inset-y-0 left-0 z-20 flex w-[min(90%,320px)] flex-col overflow-hidden bg-white shadow-xl transition-[transform,opacity,width] duration-300 ease-out lg:order-first lg:static lg:shrink-0 lg:shadow-none ${panel ? "translate-x-0 border-r opacity-100 lg:w-[292px]" : "pointer-events-none -translate-x-full border-r opacity-0 lg:w-0 lg:translate-x-0 lg:border-r-0"}`}>
      ...
      <div className="flex h-[65px] shrink-0 items-center justify-between border-b px-4"><div className="flex items-center gap-2"><History className="size-4 text-primary"/><h2 className="text-sm font-bold">{planningWorkspace ? "Planos" : "Sessões do COO"}</h2></div><button aria-label="Fechar histórico" title="Fechar histórico" onClick={() => setPanel(false)} className="grid size-9 place-items-center rounded-lg hover:bg-surface-muted"><X className="size-4" /></button></div>
