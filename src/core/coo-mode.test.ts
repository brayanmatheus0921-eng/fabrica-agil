...
import { assertApprovedPlanForAction, resolveCooMode } from "./coo-mode";
...
test("execução e acompanhamento dependem das tarefas do plano ativo", () => {
  assert.equal(resolveCooMode({ workshopStage: "FOLLOW_UP", hasCompletedDiagnostic: true, activeTaskCount: 4, pendingTaskCount: 2 }), "EXECUTION");
  assert.equal(resolveCooMode({ workshopStage: "FOLLOW_UP", hasCompletedDiagnostic: true, activeTaskCount: 4, pendingTaskCount: 0 }), "FOLLOW_UP");
});
